...
- [x] Splitwise court conversion fee: add Admin Splitwise setting `court_conversion_fee_percent` (default 1.00) and apply it only to COURT Splitwise expense totals, leaving SHUTTLECOCK math unchanged (tests: `tests/splitwise-utils.test.ts`, `tests/admin-tab-extraction.test.ts`; plus `npm test` + `npm run lint` + `npm run typecheck`; manual browser verification required for saving the Splitwise setting and checking a created Splitwise court record amount).
...
- [x] Phase 02 tab extraction: Splitwise + Emails moved into keep-mounted tab components with `page.tsx` finishing the eager/visited shell (`splitwise` eager on first load, `emails` lazy until first visit). Test rationale: manual parity verification required because Splitwise preload and Email rerun persistence are user-visible; validated with `npm test -- tests/admin-formatters.test.ts tests/admin-email-preview-rerun.test.ts`, `npm run lint`, and `npm run typecheck`.
- [x] Phase 03 shell reduction: page.tsx becomes a thin mapped tab shell while preserving local `activeTab`, keep-mounted visited tabs, and the current eager preload set. Test rationale: source-level Vitest assertions lock the shell contract because the repo has no DOM harness, and a manual `/admin` browser parity pass is still required for preload feel, tab highlighting, and hidden-tab persistence.
- [x] Milestone closure fix: Email Preview loaded messages are shared through the admin shell so Automation manual ingestion can include preview-derived Not Ingested message IDs again (tests: `npm test -- --run tests/admin-tab-extraction.test.ts tests/admin-email-preview-rerun.test.ts tests/admin-ingestion-run-route.test.ts`; plus `npm run lint` + `npm run typecheck`).
