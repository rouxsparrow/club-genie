...
export default function AccountsTab() {
  ...
  const refreshAll = async () => {
    ...
    const [sessionData, accountsData] = await Promise.all([
      adminFetch<AdminSessionResponse>("/api/admin-session"),
      adminFetch<AccountsResponse>("/api/admin/accounts"),
    ]);
  ...
  const createAccount = async () => {
    setMessage(null);
    const data = await adminFetch<{ ok: boolean; error?: string }>("/api/admin/accounts", {
      method: "POST",
  ...
  const saveAccount = async (accountId: string) => {
    setMessage(null);
    const data = await adminFetch<{ ok: boolean; error?: string }>(`/api/admin/accounts/${accountId}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  const resetPassword = async (accountId: string) => {
    ...
    const data = await adminFetch<{ ok: boolean; error?: string }>(
  ...
  const changeMyPassword = async () => {
    setMessage(null);
    const data = await adminFetch<{ ok: boolean; error?: string }>("/api/admin/account/change-password", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
